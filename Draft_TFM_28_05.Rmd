---
title: "Master Thesis Code - “Beyond the Zone: Assessing Spillover Effects of Madrid Central on Air Pollution”"
...
The objective of my thesis is to identify the existence (and if so, measure the magnitude) of geographical spillover effects of the Madrid Central low emission zone (LEZ, 2018) from an empirical perspective. In this markdown file, I present the code used to reach the conclusions presented in the dissertation “Beyond the Zone: Assessing Spillover Effects of Madrid Central on Air Pollution” (hereinafter "the main document").

The report is structured as follows. Section 1 includes the pre-processing of the data (both pollution measures and information about pollution monitoring stations, inside the City of Madrid and outside the City but within the Community of Madrid). Section 2 performs a descriptive analysis of pollution measurements by station, location (i.e. inside as opposed to outside the City) and year, as well as of the location of stations with respect to the LEZ borders and the perimeter of the Community. Section 3 presents visually the identification strategy we will use to isolate the causal effect of Madrid Central on pollution outside its perimeter. Section 4 introduces our control (weather) variables, which involves data wrangling, imputation of missing values and descriptive analysis by means of plots. Section 5 includes the Difference-in-Differences (DiD) models used to answer our research question, which follow some pre-modelling feature engineering and assumption-checking visualizations. Finally, Section 6 and 7 include robustness checks and the appendix, respectively.

We begin by loading the necessary libraries for our analysis. Some of them are used for data visualization and manipulation, such as `tidyverse`, `ggplot2`, `ggdag`, `lubridate` or `forcats`. Others are used for geospatial analysis, mapping and visualization, namely `geosphere`, `climaemet`, `sf`, `mapview`, `ggmap` and `maptools`. Additionally, we need libraries for statistical analysis, in our case `plm`, `lmtest`, `estimatr`, `fixest` and `mice`. Lastly, we load libraries for formatting and output, such as `stargazer`, `cowplot`, `corrplot`, `knitr` and `kableExtra`. We use `vistime()`and `highcharter()` to display the policy timeline in the Appendix.
...
We download hourly pollution data from the [data portal of the City Council of Madrid](https://datos.madrid.es/portal/site/egob/menuitem.c05c1f754a33a9fbe4b2e4b284f1a5a0/?vgnextoid=f3c0f7d512273410VgnVCM2000000c205a0aRCRD&vgnextchannel=374512b9ace9f310VgnVCM100000171f5a0aRCRD&vgnextfmt=default) for different pollution monitoring stations in the City of Madrid (24) for all months from the year 2015 up to and including 2022 (available since 2001).

We first create a loop to read pollution data for all years and apply data cleaning to get to a dataframe called 'all_city_data_clean'. The data cleaning steps include renaming and selecting relevant columns, transforming the data from wide to long format, filtering the data for NO~2~ (nitrogen dioxide, measured in micrograms per cubic metre or  µg/m<sup>3</sup>) measures and adjusting variable types. We then aggregate the data to the monthly level ('city_monthly').

Also, we are also interested in checking whether there are any stations with less than 18 observations per day and 280 observations per year. We find that only the pollution monitoring station in Arturo Soria (station code 16) has less than 280 observations per year (264). However, we can see that this is only the case for 2022 and we will not use observations from this year for reasons introduced later. Hence, we do not remove the data yet to ensure comparability.

```{r}
# `For` Loop for hourly data for all months in all years
file_names <- c(paste0(c("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), "_mo15.csv"),
                ...
                paste0(c("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), "_mo19.csv"),
                paste0(c("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), "_mo20.csv"),
                paste0(c("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), "_mo21.csv"),
                paste0(c("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), "_mo22.csv"))
...
# Check for stations with less than 18 observations per day and 280 observations per year 
stations_to_exclude <- all_city_data_clean %>% 
  ...
  filter(n_obs_per_day < 18 | n_obs_per_year < 280) 
...
We employ a similar strategy to read pollution data from pollution monitoring stations outside the City of Madrid from 2015 to 2022. One can find hourly data for 24 stations inside the Community of Madrid (excluding the Capital) since 2005 [here](https://datos.comunidad.madrid/catalogo/dataset/calidad_aire_datos_historico). We apply virtually identical cleaning and aggregation steps to create our dataframe aggregated to the monthly level.
...
Then we extract the longitude and latitude of all stations in the network. We generate a variable representing the distance of each one to Plaza del Carmen (only station inside the LEZ and policy centroid, see section 2.3). We then arrange all stations by increasing distance from the city centre.
...
city_stations_clean <- city_stations_clean %>%
  # Use the distHaversine function to calculate the distance between each station and Plaza del Carmen in kilometers
...
For each station, we have information on their code (to later join station data with pollution data), address, location (longitude and latitude) and distance to the centre. Note that we also see that pollution monitoring stations are of several types:
...
com_stations_clean <- com_stations_clean %>% 
  ...
  # Remove the 'area_type' and 'station_type' columns and rename the new column to 'station_type'
...
We combine the data for all 48 stations in the Community of Madrid after selecting the relevant variables into a dataframe called 'data_all_stations'.
...
### 2.2. Comparison of average yearly pollution and compliance levels with Directive 2008/50/EC between in-City and out-of-City stations
...
To check how have average pollution levels evolved between 2015 and 2022 inside vis-à-vis outside the City of Madrid, we aggregate both sets of data (in-City vs. out-of-City) by year and compute the mean. We then combine the data and pivot our data to wide format.
...
We see that only from 2020 the number of times per year where hourly pollution levels exceeded 200 µg/m<sup>3</sup> in Madrid was less than 18. Outside the city of Madrid only 2015 saw a violation of the rule for hourly NO~2~ concentrations as prescribed by the Directive.


### 2.3. Visualizing the location of pollution monitoring stations

#### Perimeter of Madrid Central Low Emission Zone (LEZ) and pollution mearusing stations stations in the City of Madrid
...
# Create binary variable to denote if station is within the ZBEDEP 
stations_in_LEZ$LEZ <- 
  ifelse(is.na(stations_in_LEZ$AMBITO_GEO), 0, 1)

# Display the stations on the map with LEZ perimeter
...
#### Specification of rings based on distance from Plaza del Carmen (policy centroid)

With the goal of quantifying the spillover effects of the policy beyond the borders of the LEZ, we define a set of **rings** which segment all 24 stations in the Community of Madrid network according to their distance from the Madrid city centre. 
...
```{r}
# Specify cutoffs and assign labels
...
Next, we set a bounding box that encompasses the Community of Madrid by defining its minimum and maximum longitude and latitude coordinates. We then retrieve the map from [Stamen Maps API](https://www.r-bloggers.com/2018/10/getting-started-stamen-maps-with-ggmap/) using `get_stamenmap()`. 

The map is plotted using `ggmap()`, which allows us to add the map tiles to a 'ggplot2' object. The 'extent' argument is set to "normal" to ensure that the aspect ratio of the map is preserved and 'maprange' is set to "FALSE" to prevent `ggmap()` from adjusting the axes of the plot. Each station is a point on the map and its corresponding ring is coded through the color aesthetic.
...
Importantly, we remove rural (background) stations from the dataset with out-of-Capital stations because, as we saw before, these stations are intrinsically different from those in urban settings even outside the city. They are located in places with very little traffic and thus, when put together with other stations outside the City of Madrid, differences in pollution levels are falsely exaggerated.

We combine our dataframes and replace NAs in the 'town_name' column by 'Madrid' since the 'city_stations_clean' dataframe lacks this column in contrast with 'com_stations_clean'. We only want observations for January and July for each year since we are also interested in analyzing whether there are significant differences between two given months of the year.
...
full_data_plot <- full_data_complete %>%
  ...
  # create column with 4 different labels with combinations of 'type' and location (in- vs. out-of-City)
...
First, there does seem to be a clear difference between urban and suburban stations, both when considering stations inside and outside the City of Madrid, urban stations (blue) always have higher average pollution levels than suburban ones (red), regardless of whether the station is inside or outside the Capital.
...
Let's take a step back. We are trying to assess how the Madrid LEZ affects the pollution levels *outside* the perimeter of the LEZ. We make use of the following **Causal Directed Acyclic Graph (DAG)** to depict visually our identification strategy aimed at isolating and quantifying this effect by relying on causal inference theory.

We use the `dagify()` function from the `ggdag` package. Each node represents a variable in the causal model and the edges represent the causal relationships between them. We specify the **treatment** as the Madrid LEZ and the **outcome variable** as pollution outside the LEZ. We then define three possible mechanisms or ways by which our treatment may affect our outcome, giving rise the following **mediators**: 

(1) <u>Redirection of traffic from areas subject to restrictions to areas without restrictions</u>, thereby *increasing* pollution outside LEZ

(2) <u>Increased use of public transport to avoid restrictions on private vehicles</u>, thereby *decreasing* pollution outside LEZ (as well as inside)

(3) <u>Replacement of highly polluting with "greener" vehicles by drivers to avoid fines</u>, thereby *decreasing* pollution outside LEZ (as well as inside)

Finally, we theorize that there may be some potential **confounders** which could bias our estimate by causing a change in pollution levels not attributable to the LEZ. First, macroeconomic conditions may affect the use of private vehicles (increasing the relative use of public transportation) and the willingness or ability to purchase new ones, thus affecting mediators (2) and (3). Second, large public works in the city centre could have an important effect on channels (1) and (2) by "forcing" people to take alternative routes to avoid traffic jams or by encouraging people to use the subway, respectively (see Section 6). Third and last, meteorological conditions may affect NO~2~ concentrations independently from the existence of restrictions, thus posing a threat to our identification.

While, as we will see, the impact of both public works and economic indicators is accounted for by fixed effects in our model, weather effects may escape the control of the intersection of unit and time fixed effects and thus need to be incorporated explicitly into our model.

We set the color of each node according to its type (outcome, treatment, mediator or confounder) and use the `scale_color_manual()` function to manually set the colors for each node type.

```{r}
# Set the theme of the plot to a directed acyclic graph (DAG) theme
theme_set(theme_dag())
...
After cleaning our data, we seek to match pollution monitoring stations with the closest weather station for each. To that end, we extract a matrix of coordinates for all air pollution monitoring stations *and* weather stations in the Community from the 'data_all_stations' and the newly created 'weather_stations' dataset, respectively. We then calculate the distances between pollution and weather stations using the Haversine formula and match each air pollution station to its closest weather station by adding a new column to the 'data_all_stations' dataframe.
...
While we obtained data from 2022 to show the evolution of pollution levels using the most recent observations, pollution data after June 2021 will not be used in our model for reasons outlined later and in the main document. Hence, we will only access weather data from 2015 until the end of 2021 to keep all observations for all seven years.
...
# Make transformations: select, adjust classes and create new columns
...
### 4.3. Imputation of missing values in weather varaibles

Only by looking at the dataframe, we see that we face the problem of missing values in our weather variables. We will use **multiple imputation**, which relies on the distribution of the observed data to estimate multiple plausible values that reflect the uncertainty about the missing data. While it generally performs better than zero- or mean-/median- imputation and is most likely superior to dropping observations altogether, we need to check a few assumptions first.
...
# Calculate the percentage of missing values for each numeric variable in the dataframe (weather variables)
...
ggplot(na_counts, aes(x = weather_station_name, y = na_count)) +
...
We see that the stations in Buitrago del Lozoya, Ciudad Universitaria and Somosierra have a considerably high number of missing values compared to the other stations. This suggests that the missingness of 'min_pressure' (and for the other variables for which we repeated this process) might not missing at random.

Nevertheless, there are two main reasons why we will proceed with the multiple imputation of our weather covariates. 

First, by including station fixed effects in our models (Section 5), we can think of missingness as an additional attribute that varies across stations besides, for instance, the amount of traffic. Hence, we want to ensure that we do not observe more missing values at specific dates. For that, we do a similar plot but this time we group by date rather than station:
...
While March of 2018 seems to have an extraordinarily high number of missing values, there does not seem to be any pattern indicating missingness is correlated with time. We hence have a more or less balanced panel.

Second, below we see that the imputed distributions of the 'min_pressure' variable are rather similar to the original one in the plots below. In order to visualize the imputed values for this specific variable, we use the package `mice()` (standing for 'Multivariate Imputation by Chained Equations') with three different algorithms: Predictive Mean Matching ("pmm"), Lasso Regression ("lasso.norm") and Classification And Regression Trees ("cart"). We use ten imputations for each missing value (m=10).
...
```{r}
# Create histograms for the distribution from each algorithm and compare with original one
...
Although all algorithms yield approximately equivalent distributions, PMM seems to yield the closest to the original distribution. Due to this and the relatively lower required computational time, we choose PMM to impute all our weather variables using 10 imputations to generate each missing value.

For reference, PMM is used in a chained equation approach, whereby in each iteration each variable with missing values is imputed by finding the nearest observed value to the missing value. It relies on the use of a regression model that includes all other variables in the dataset (both observed and imputed). 
...
To analyse more specific information than the one provided by the visual inspection of the imputed distributions above, we see that the 5-number summary of minimum pressure is very similar to the one of the original variable:
...
After having imputed our eight weather variables, we aggregate them by month. We find the mean for the temperature- and wind-related variables, while we compute the sum for the rain following common practice. Finally, we  combine the information of the 'min_pressure' and 'max_pressure' covariates into a single variable ('avg_pressure') by averaging them out.
...
i. **(Average) temperature** (ignore maximum and minimum for now until the next section)

ii. **(Total) rain**

iii. **(Average) direction of maximum wind speed**

iv. **(Average) wind speed**
...
Importantly, we replace missing values for the 'ring' column in the newly created dataframe by "Ring 6: Excluded". The reason is that all missing values corresponded to the weather station in Somosierra since it was not selected as the closest weather station for any of our pollution stations in the computation we performed before. As such, weather observations taken in Somosierra had no information about the ring and we thus had to assign the last ring to them manually.

We then use boxplots to compare the distributions of our variables in a given year (2019 for instance, as it is the first year of full implementation of Madrid Central). We select only average temperature to avoid redundancies, pivot our data, recode the newly created weather variable to select its levels manually and create the plot.
...
# Create the plot
weather_all %>%
  # Remove min and max temperature variables for simplicity
  ...
  # Transform variable with weather variables as categories into factor and recode (assign names)
...
We saw some differences with data aggregated at the yearly level. However, we want to to see if there are any interesting patterns in the evolution of our weather variables for a given year (again, 2019) varies across stations.
...
With all the necessary data loaded and (almost) properly transformed, we are ready to kick off the modelling stage. We will employ a **Difference-in-Differences (DiD)** design to uncover whether there were any spatial pollution spillovers as a result of the introduction of restrictions in Madrid Central, and if so, how big they were and how far out they are significant.
...
We can now plot the evolution of average pollution levels. If the assumption is met, we should not have the *trends* in the yearly averages diverging prior to the policy implementation. 
...
First, we seem to have parallel trends. Despite some differences in the slopes between the first four rings and the last two, pre-intervention trends are pretty parallel and thus we can assume that any differences in pollution levels after the implementation were solely due to the policy. Put differently, in the absence of intervention, we assume that nothing would have changed from what already had already been observed (i.e. similar slopes but different intercepts reflecting station-specific fixed effects).
...
Note that in Section 2.4 we removed rural stations from our data (6 out of the 8 stations in the last ring) in order to reflect more accurately the differences between urban and suburban stations in our network. Now we remove the *remaining* 2 stations from the Ring 6 (Aranjuez and Villarejo de Salvanés) which happen to be background suburban and traffic suburban stations, respectively. 

We do this because, when running our models, we want control and treated stations to be as similar as possible and including these in the former would threaten that goal. We then remove any levels of the 'ring' factor (the one referring to Ring 6 in this case) that are not present in the remaining data.
...
#### Merging final dataframes to capture all information necessary to run the model

Now we perform our last merging of dataframes in order to get all the information we need for the model into 'did_df'. The dataframe 'model_pollution_df' contains information about dates, pollution and stations (location, type, distance from the centre, ring and closest weather station) while 'weather_full_final_a' has data on dates, weather stations and weather variables. We can hence join them by the name of the weather station. This variable has different names in each dataframe and so we rename it in the first dataframe before merging it with the second:
...
#### Pre- vs. post-treatment dummies

With our final dataframe, we are ready to create our variables for the DiD models. We first need a variable taking the value of 1 for observations happening after the introduction of Madrid Central measures (i.e. after and including December 2018), and a value of 0 otherwise. 
...
Next, we will create a series of treatment dummies for each ring to include in our model. We mutate four new columns in the dataframe that take on a value of 1 if the corresponding row is in the treated group for that ring, and 0 otherwise.
...
The logical next step within the DiD framework is creating new variables that identify whether a given observation is in the post-treatment period for each treatment group. This is an interaction term that "activates" each given treatment in the period after the LEZ's entry into force.
...
#### Create 'station_id' variable 

Since our units of analysis are pollution monitoring stations, we would like to create a variable ('station_id') that uniquely identifies each station in the Community of Madrid air quality network. We simply pastes the station's unique code with the code of the town it belongs to.
...
#### Correlation between temperature controls and the target
...
To be sure we are including the most relevant one to explain pollution, we compute the correlation of each of the three variables with our target.

```{r}
# Calculate correlation coefficients
correlations <- cor(did_df[c("pollution_level", "avg_temperature_monthly", "min_temperature_monthly",
...
# Remove 'min_temperature_monthly' and 'max_temperature_monthly'
...
Average temperature has the highest correlation with pollution - over 0.4 in absolute value (-0.4). Therefore, we remove the other two variables.

We also remove variables containing redundant information from our dataset.

#### Correlation among all weather variables and with the target

Since we now have the final list of our five weather controls, we can check their degree of association with the target variable measured by Pearson's correlation coefficient. Hence, we select the six variables (target and five weather controls), compute the correlations and make some adjustments to our correlation plot:

```{r}
# Select the columns containing the variables of interest
...
# Add the title to the plot using mtext()
mtext("Correlation Plot of Weather Controls and Pollution Level (Target)", side = 3, line = -0.1, cex = 1.1)
```

We see that, among all weather variables, average temperature and average wind speed seem to be the most correlated with pollution. Both have moderate negative linear associations with the target. Pressure, rain and wind direction are barely associated with the target since their correlation coefficients are close to 0. 

We also see that correlations *among* our weather variables are not excessively high, the largest one being the (obviously negative) association between average temperature and total rain measured on a monthly basis. 

While some variables may not be highly correlated with the target, we still include them because they may have explanatory power *in conjunction with* other variables. More importantly, we should be concerned that some of these variables may be responsible for some variability in the dependent variable that must be accounted for to avoid biased estimates in our model. 

### 5.4. Summary of target, explanatory and control variables in the model

We create a table summarizing the variables in our model, including their names, description and basic summary statistics (mean and standard deviation).
...
# Remove the row names from the summary data frame
...
### 5.5. Manual calculation of Average Treatment Effect 
...
It is time to run our DiD regressions. In practice, DiD identification strategies are implemented through **fixed-effects models**. We run five different ones, each with an increasing number of predictors and thus degree of complexity compared to the previous one. A comparison of the results from the five models is offered at the end of this section.

These models rely on within-group transformations ('model' argument set to "within"), which will allow us to account for individual-specific unobserved heterogeneity (i.e. unit fixed effects) that may affect the dependent variable ('pollution_level') by subtracting the average of each station's observations from the observations of that given station. This transformation effectively removes the time-invariant heterogeneity across stations, leaving only the within-individual variation to be explained by the regressors and allowing different intercepts for different stations.

First of all we transform the month column to a string so that we are later able to remove the 11 levels of that variable from the summary of our five models for the sake of simplicity. We transform the time, treatment and interaction terms into factors.
...
All our variables seem to be significant and the *R-squared is 0.67*, meaning 67% of the variability in the pollution level is explained by the variables in our model.
...
Our treatment effect (coefficient for 'POST_treat_1') for the closest four stations to the city centre (those within a radius of 2.5 kilometres of Plaza del Carmen) is around **-6.2**, which is close to the result of our manual calculation of the ATT. This a very clear negative (causal) effect of the implementation of restrictions within Madrid Central on the pollution levels around these four stations.

We also see that the levels of the 'month' variable have negative coefficients, thus reflecting the insights introduced in Section 5.1 (Parallel Trends) whereby aggregate pollution levels show a negative trend. In absolute value, the magnitude of this decrease seems to be strongest for the months of May and June and weakest for December as seen above.

We can investigate further how the estimates for Ring 1 vary with time. Instead of using month, we will use the year to avoid having an excessive number of time points. For that we use a different package (`feols()`) to estimate normal OLS with our station and (now) year fixed effects (see Section 6 for more on year fixed effects). We choose the year of implementation, 2018, as reference (not shown in the plot).
...
```{r}
# Convert the "year" into a factor and relevel it with the reference level set to '2018'
did_df_feols <- did_df %>%
...
For Model 2 we still take our four most central stations as the only treated units. Yet, apart from controlling for the month and station, we add our weather covariates. We also remove the treatment dummies from the regression model as the `plm()` package produces the same results with and without them (i.e. it understands the treatment dummy to be implicitly there).
...
It seems that all controls are statistically significant at the 0.05 significance level except average pressure and average temperature. Our adjusted R-squared (which penalizes the inclusion of irrelevant variables into the model) has risen to around 74.6%. This means that meteorological information is important at explaining the variability in pollution levels.

Our treatment effect for the closest four stations to the city centre is now around **-4.8** with a **p-value of 0.002**. That is, the policy has led to a decrease of 4.75 µg/m<sup>3</sup> in NO~2~ concentrations inside and within the immediate neighbourhood *after controlling for the effects of meteorological conditions*. This is lower than the effect found in Model 1 in absolute value. This reduction means that, by incorporating weather controls, we were able to "strip away" some effects on pollution that were actually not caused by the restrictions in Madrid Central. 


#### Model 3: first and second treatment rings with weather controls

Model 3 retains the same controls (time and station fixed effects, and weather), but we now incorporate the second treatment ring (stations within 2.5 and 10 kilometres from Plaza del Carmen) in addition to the first.
...
The adjusted R-squared is virtually the same. Our treatment effect for stations in Ring 1 is now around **-5.2** (**p-value = 0.001**) and the treatment effect for stations in Ring 2 is about **-1.2** (**p-value = 0.1**). There is an evident decrease in the absolute magnitude of the coefficient for Ring 2 compared to Ring 1, with the effect being insignificant. Hence, the reduction of NO~2~ in the air was much weaker in the stations farther away from Plaza del Carmen than in those stations in its immediate vicinity (or the centroid station itself).
...
Again, the R-squared has not changed much by adding new stations, which makes sense since it is the addition of weather controls and not the incorporation of new units into the treatment that enhances the explainability of the model. Our treatment effects for Rings 1, 2 and 3 are now: **-5.7** (**p-value = 0.001**), **-1.7** (**p-value = 0.1**) and **-0.9** (**p-value = 0.4**), respectively. 

Only the effect of the policy for stations in Ring 1 is significant; the effects are insignificant for those in Rings 2 and 3, with this significance being lower in Ring 3 compared to Ring 2. This makes sense as Ring 3 stations are farther away from the "restricted area" than Ring 2 stations. This means that, if there were any important spillover effects, these would be stronger and more likely to be statistically significant in stations closer to where the policy is actually active.


#### Model 5: four treatment rings with weather controls

Our last model is the most complete as it incorporates all controls and fixed effects, as well as all stations aside from those in the control group (stations within 20 and 40 kilometres from Plaza del Carmen).
...
This last model is equally explanatory as the last three and the treatment effects for Rings 1, 2, 3 and 4 are around **-5.6** (**p-value = 0.003**), **-1.6** (**p-value = 0.2**), **-0.8** (**p-value = 0.6**) and **0.4** (**p-value = 0.8**). Again, from Ring 2 onwards the treatment effects lose statistical significance with a decreasing level of significance as our stations are located farther away from the centre. The coefficient and statistical significance for the treatment consisting of Ring 4 stations are as expected: close to 0 (even positive!) and quite high, respectively.
...
Likewise, the coefficient for 'POST' (about -5.2 for Model 5) represents how much the average outcome of the control stations has changed in the post-treatment period. It is significant in all our models, implying pollution has decreased over time in the control stations and thus confirming the negative trend seen when investigating parallel trends. 

**Visualization of treatment effects**

We can plot the coefficient estimates for the interaction term (i.e. treatment effect) for each of the four rings and their respective confidence intervals. First, we extract the coefficient estimates and standard errors, after which plot the contents of a newly created dataframe.
...
ggplot(df_plot, aes(x = treatment_ring, y = coefficient)) +
  ...
  labs(x = "Treatment Ring", 
       ...
       title = "Treatment Effect Coefficient Estimates by Ring") +
  ylim(-10, 5) +
  theme_light()
```
...
Lastly, we export the results of our five models. To do that, we first create a vector of month dummies in order to exclude them from the model (since they do not provide useful information about the significance of spillovers). We also set the names of the variables for better readability in the regression table. Next, since we do not want the original model object ('m5_quad') but rather the model with the correct (clustered) standard errors, we create our own function that calculates these for all four models (and for those used to assess the robustness of the model in the rest of this document). This is done because inserting our previously created model objects with clustered standard errors (e.g. clustered_test_m5) into the `stargazer()` function returned an error once more than 3 models were included.
...
# Create the list of five models to include
...
We will start by introducing several changes to the model with the highest number of regressors (Model 5), consisting of aggregating data at the weekly level, using a different specification of treatment rings, modifying our controls and utilizing a different model set-up.

We then run a placebo test, which is often used to test the validity of causal inference by examining the effect of the intervention on the outcome variable at a time or place where the treatment is not expected to have an effect. 

### I. Modificiations to the most complete model

#### A. Weekly Aggregation of Model Data  

The first robustness check consists of testing whether aggregating at the weekly (rather than monthly) level has any impact on the significance of the policy impact.

We repeat the steps followed in the first sections of this document, although we now aggregate both pollution and the weather variables at the weekly level.
...
```{r}
# Aggregate city data to weekly level
...
Concerning weather data, we only need to modify the last step of Section 4 in the code above (having done imputation) and change the level of aggregation. 
...
Finally we run Model 5 on weekly data. We include all four treatment rings and our weather covariates. 
...
# Export the results using stargazer
...
We see that we obtain essentially the same results: only the average pollution levels of the stations in the first treatment ring seem to be significantly affected by the implementation of Madrid Central (the estimated effect is around -6 µg/m<sup>3</sup>). 

Now we have 16,178 observations, compared to the 3,120 we had when aggregating by month. This could have potentially masked important variability or information, for instance by amplifying measurement error. However, we have shown this not to be the case: we can state the results found in Section 5 are robust to a different level of aggregation.

#### B. Alternative model settings using two-Way fixed effects 

Again, rather than testing the robustness of our results as we do in the next sections, we want to show that changing the model setup (while still using the `plm() package`) does not change results significantly.
...
In order to do that, we repeat the transformations we used before and create our treatment rings following this alternative specification.
...
Our estimates do not seem to be very different from the ones in the original model. We can repeat our plot with the confidence intervals for each (new) treatment ring to assess whether the new specification actually leads to different results:
...
```{r}
# Set up model for only Ring 1 stations
...
# Create the list of four models to include
...
Reverting back to month fixed effects, we re-run Model 5 while controlling for the type of station:
...
Between April and November of 2019, the City Council undertook the comprehensive remodelling of Gran Via. Since this station is the main avenue crossing the LEZ, we want to know if controlling for it (and thus for one potential confounder as seen in our DAG above), makes any difference for our estimates.
...
variable_labels_gv <- c(
  ...
  "wind_direction_monthly" = "Average Monthly Wind Direction",
  "avg_pressure_monthly" = "Average Monthly Pressure",
  "as.factor(GRAN_VIA)1" = "Gran Via Remodelling")
...
The estimates change slightly compared to the original model but not significantly. However, we see the variable 'GRAN_VIA' is actually significant. The presence of construction works in Gran Via during the 8 months prior to the introduction of the LEZ had an average effect on the pollution level of around -3.6 µg/m<sup>3</sup> (compared to its absence), holding all other variables in the model constant. This means that there is an *overall decrease* in average pollution in Madrid associated with this particular instance of public works.
...
There are several explanations for this. Since only month fixed effects capture seasonal variation in pollution levels, the significant and larger estimate for the model including them suggests that there is a significant difference in pollution levels throughout the year.

Another important difference with respect to the month-fixed-effects model is that the (adjusted) R-squared has dropped from around 0.75 to 0.67. Hence, month fixed effects do seem to account for more variability in the target than year fixed effects.

#### H. Model with quadratic specification for weather covariates
...
First, the coefficients for the meteorological covariates in their linear form have changed quite a lot after the inclusion of squared terms. For the quadratic form, for instance, the square of total rain is significant and positive. This indicates a concave relationship between total rain and pollution, assuming all other variables in the model are held constant. This means that higher precipitation is associated with higher pollution levels, and this effect becomes stronger as total rain increases. Concerning significance levels, none of the variables that were insignificant in the linear model gain significance in the quadratic one, implying non-linear effects are not that strong.

Second, the estimates for the effect of the policy for our four treatment rings are close but slightly bigger in absolute value in the quadratic. 

We will keep the linear model as our reference for Model 5 since only one quadratic term is significant and our treatment effect estimates are equivalent to the ones in the linear model, while the latter does not pose such a high risk of multicollinearity as the quadratic model.

### II. Falsification test I: testing the effects of the treatment on the outcome in a 'placebo' time period

Let's falsify our results. We start by testing the significance of our treatment effect (within the 2.5-kilometre radius of Plaza del Carmen since we found no further spillover effects) in years *prior* to the implementation of Madrid Central. This will allow us to assess whether the estimated treatment effect we found is driven by the treatment itself or rather by other factors that may be present in the data. In other words, for our treatment effect to be *robust* we would like to find a lack of statistical significance on our 'POST_treat_1' variable when we specify the treatment took place in November 2015.

We reproduce the steps used above, but this time with data from 2012 to June of 2018 to avoid any anticipatory effects going up to November of 2018 (date of implementation). We also only use the Model 2 specification from Section 5 as it is only for the first treatment ring that we found statistical significance.
...
We take the last dataset and join it with our previously created dataset with new variables for each station. Just like we did for the estimation of our actual treatment effect (back then with June 2021), we only keep observations up to June 2018. We also remove Ring-6 stations.
...
We load weather data for the new time window and make the appropriate transformations. We impute missing values and group by month just like we did before. 
...
We create our dataset 'did_df_r'. This includes pollution and weather data which we will use to run our model. We create the dummies that go into our model - time, treatment ring (again, only the first) and the interaction between the two - and transform them into factors. We make some more modifications just like we did before.
...
### II. Falsification test II: testing the effects of the treatment on a 'placebo' outcome 
...
for (file_name in file_names) {
  ...
  # Append the cleaned data to the 'all_acus_data_clean' dataframe and drop NAs
...
for (month in months) {
    ...
    # Append the cleaned data to the 'all_acus_data_clean_2016' dataframe and drop NAs
...
for (month in months) {
    ...
    # Append the cleaned data to the 'all_acus_data_clean_2015' dataframe and drop NAs
...
Just like we did with the pollution monitoring station network in Madrid, we download information about the location (address and coordinates) and unique identifiers of these stations.

There are 31 in total, some of which overlap with pollution measuring stations (for instance the policy centroid, i.e. Plaza del Carmen) and some which do not.
...
Fortunately, the effect of the intervention is found to have insignificant effects on the average noise level in Ring 1 stations. This increases the plausibility of observed effects on air pollution being truly attributable to the LEZ. This strengthens the validity of our research design.

## 7. APPENDIX

### Weighted imputation of missing values in the weather controls

As we saw in Section 4.3, the assumption required for multiple imputation (MAR) does not seem to be satisfied as missingness appears to be related to the station. With the hope of improving the reliability of the imputation procedure, we include weights. Following the approach in Gomila and Clark (2020), larger weights are assigned to observations that have larger probabilities of being missing and smaller weights are assigned to observations with lower chances of missingness (IPM, or *Inverse Probability Weighting*).
...
We now take the example of minimum pressure and create a variable that takes a value of 1 for available observations and 0 for missing values. We then regress this new variable on the column capturing the weather station using a logistic regression, after which we generate probabilities for missingness and the corresponding weights (inverse of probability of missingness). 

We then perform imputation by [Weighted Predictive Mean Matching (PMM) or Weighted Normal Linear Regression](https://search.r-project.org/CRAN/refmans/miceadds/html/mice.impute.weighted.pmm.html). We incorporate the imputed values into the original dataframe 'weather_full_final_imp'.
...
# Model missingness of minimum pressure conditional on the weather station
...
# Calculate the inverse of the predicted probabilities
...
Before we look at the imputed distribution, we confirm that it is actually different from the imputed one using PMM:
...
We see that the five-number summary and distribution of minimum pressure after imputing by PMM with weights is very similar to both the original and the (unweighted) PMM-imputed one. Hence, retain the "normal PMM imputation" for all our weather variables in the model.
...
### Access rules according to vehicle pollution potential
...
# Define description
descriptions <- c("First phase of the transition period. No active cameras, 
                  with police officers monitoring and enforcing rules", 
                  "Automatic monitoring system with cameras at the LEZ perimeter. 
                  Warnings for non-compliance sent by post",
